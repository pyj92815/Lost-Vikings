...
{
	IMAGEMANAGER->addFrameImage("화살붕괴", "./image/Characters/arrow_break_x3.bmp", 128, 32, 4, 2, true, RGB(255, 0, 255));
...
void playerManager::render()
{
	...
	for (int i = 0; i < _vArrowBreakTag.size(); i++)
	{
		_vArrowBreakTag[i]->breakImage->frameRender(getMemDC(), _vArrowBreakTag[i]->breakImage->getX(),
			_vArrowBreakTag[i]->breakImage->getY(), _vArrowBreakTag[i]->frameX, 0);

		_vArrowBreakTag[i]->timeCount++;

		if (_vArrowBreakTag[i]->frameX < _vArrowBreakTag[i]->breakImage->getMaxFrameX() && _vArrowBreakTag[i]->timeCount % 5 == 0)
		{
			_vArrowBreakTag[i]->frameX++;
		}
		else if (_vArrowBreakTag[i]->frameX == _vArrowBreakTag[i]->breakImage->getMaxFrameX())
		{
			_vArrowBreakTag.erase(_vArrowBreakTag.begin() + i);
		}
	}
		...
		if (IntersectRect(&temp, &_baleog->getBaleogRect(), &_wo->get_vTrap()[i].rc))
		{
			...
			for (int j = 0; j < _baleog->getVArrow()->getVArrow().size(); j++)
			{
				...
				if (IntersectRect(&temp2, &_baleog->getVArrow()->getArrowRect(j), &_wo->get_vTrap()[i].rc))
				{
					...
					arrowBreakTag *abt = new arrowBreakTag;

					abt->timeCount = 0;
					abt->breakImage = IMAGEMANAGER->findImage("화살붕괴");
					abt->breakImage->setX(_wo->get_vTrap()[i].rc.left);
					abt->breakImage->setY(_wo->get_vTrap()[i].rc.top);
					abt->frameX = 0;

					_vArrowBreakTag.push_back(abt);
	...
	for (int i = 0; i < _em->getVEnemy().size(); i++)
	{
		...
		for (int j = 0; j < _baleog->getVArrow()->getVArrow().size(); j++)
		{
			...
			if (IntersectRect(&temp, &_baleog->getVArrow()->getArrowRect(j), &_em->getVEnemy()[i]->getRect()))
			{
				...
				_em->getVEnemy()[i]->Hit(); //적 HP를 깍아줄 코드
